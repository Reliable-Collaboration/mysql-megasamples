#!/usr/bin/env python3
"""PLAN.md section 8.3: the pre-publication checklist, as something you can run.

  python3 scripts/prepub_check.py [--image mysql-megasamples:dev] [--skip-image]
...
def main():
    ...
    ap.add_argument("--image", default=os.environ.get("MEGASAMPLES_IMAGE", "mysql-megasamples:dev"))
    ...
    # 1 -------------------------------------------------------------------------------------
    p = run([PY, "scripts/gen_provenance.py", "--check"])
    ...
    # 4, 5, 6 -------------------------------------------------------------------------------
    cmd = [PY, "scripts/audit_assets.py", "--image", a.image] + (["--skip-image"] if a.skip_image else [])
    p = run(cmd)
