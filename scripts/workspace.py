...
"""What this project has running, and how to get rid of it.
  ...
  python3 scripts/workspace.py status     what is up: the compose stack, and any throwaway container
  python3 scripts/workspace.py clean      remove the throwaway containers, leave the stack alone
  python3 scripts/workspace.py clean --all   also `docker compose down`

Two kinds of container exist and they are not the same kind of thing:

* The **stack** -- the database and the four consoles -- is one Compose project, `mysql-megasamples`.
...
`build/dumps` and the loaders.
"""
import argparse, json, subprocess, sys

LABEL = "megasamples.transient=true"
PROJECT = "mysql-megasamples"
...
KNOWN = ("mms-build", "mms-mssql", "mms-image-test", "mms-image-test2")
