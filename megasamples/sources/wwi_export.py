...
"""Open the WideWorldImporters backups in SQL Server once, and export everything MySQL will need.
  ...
  MEGASAMPLES_ACCEPT_MSSQL_EULA=1 python3 -m megasamples wwi-export
...
from megasamples.sources import mssql  # noqa: E402
...
def main(argv=None):
    ...
    for _, bak, dataset in DATABASES:
        ...
        if not os.path.exists(path):
            sys.exit(f"missing {path}; run: python3 -m megasamples fetch {dataset}")
