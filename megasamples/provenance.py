#!/usr/bin/env python3
"""Generate the licence and provenance files from the knowledge bundle (ARCHITECTURE.md section 8).

  python3 -m megasamples provenance [--no-measurements] [--check]
...
GENERATED = ("<!-- Generated by megasamples/provenance.py from knowledge/ and manifest.yaml.\n"
...
def build_license_file(name, cfg, licences):
    lines = [f"Licence terms for the `{name}` database as redistributed by sql-megasamples.", ""]
...
def build_provenance(name, cfg, arts, record, measurements, licences):
    ...
    if not arts and cfg.get("tier") == "not-shipped":
        ...
        out += ["## Source artifacts", "",
                ...
                "no release asset exists. `megasamples/sources/bikeshare.py` fetches one month from the",
        ...
        out += ["## Source artifacts", "",
                ...
                "is the *generator*: `engines/mysql/loader.Dockerfile` fixes the source commit it is built",
...
def write(path, content, check, changed, ignore_measured=False):
    ...
    if ignore_measured and existing is not None and MEASURED in existing:
        ...
        # there (ARCHITECTURE.md section 8). Everything above it must still match exactly.
