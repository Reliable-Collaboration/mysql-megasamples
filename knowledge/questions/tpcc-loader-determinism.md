...
# Cheapest experiment
Loader image: `apt-get install sysbench`; run W=1 twice with the same seed and threads=1, compute the per-table digests with `scripts/checksum.py` excluding c_since/h_date/o_entry_d/ol_delivery_d; repeat with threads=4; grep `math.random` seeding in sysbench's Lua bootstrap (`src/lua/internal/sysbench.rand.lua`).
