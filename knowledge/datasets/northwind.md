...
**2026-09-09.** Six procedures are in the corpus (`custordersdetail` ports since the select-list aliasing rule was added; `salesbycategory` stays refused). Two defects surfaced when the PostgreSQL and SQLite ports were verified by executing the objects ([parity record](/decisions/programmable-object-parity.md)): `Invoices` computed `Salesperson` as `FirstName + ' ' + LastName`, which MySQL adds as numbers and returns 0 — the converter now rewrites `+` between strings to `CONCAT`, and the view reads `Nancy Davolio`; `sales_by_year` kept `DATENAME(yy, ShippedDate)`, which MySQL does not have, so the procedure failed on the hub itself — the converter now maps `DATENAME` (year as text, `MONTHNAME`, `DAYNAME`, other parts as their number), and the call is part of `tests/routines.yaml`, pinned from MySQL and reproduced on PostgreSQL. All 16 views and 6 procedures are verified on PostgreSQL; on SQLite the 16 views are, 9 of them without their computed decimal columns (floating-point arithmetic), and the procedures are not portable (no stored routines).
