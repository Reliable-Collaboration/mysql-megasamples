...
**Extended tier built 2026-09-03 (task X-03)**: `make dvdstore-reviews` appends `reviews` (**200,000** rows) and `reviews_helpfulness` (**4,106,382**) to an already-loaded `dvdstore`, taking it from 16.3 MB to **475.9 MB** in 16 s. Both counts are the ones recorded during research and both reproduced exactly; every review's product exists, every helpfulness row's review exists, and the star ratings are uniform at about 40,000 each, as a generator produces. The two review stored procedures (`new_prod_review`, `new_review_helpfulness`) come with them, so the database then has all four. Nothing here is baked into the image.
