...
description: DuckDB reads Parquet/CSV and emits typed MySQL-ready CSV; server-side LOAD DATA (strict) writes it, util.importTable only above 5 M rows. One pipeline for TLC, BTS, Chicago, Citi Bike and Divvy.
...
# Options considered
1. **DuckDB as reader/reshaper -> typed CSV -> `LOAD DATA` (server-side, strict; `util.importTable` above 5 M rows)** (chosen). One tool reads all four container formats (`read_parquet`, `read_csv`, zipped CSV), does the casting and column renaming in SQL, and writes a canonical CSV that the fastest MySQL bulk path consumes.
...
1. **Fetch** the upstream artifact into `./downloads/`, record `sha256`, `Content-Length` and `Last-Modified` in `downloads/<id>.meta.json`, which `megasamples/registry.py` folds into the `megasamples.datasets` table at image build or extended load (no upstream checksums exist for any of these five - see each dataset record).
...
3. **Load** into a pre-created table whose DDL lives in the repo (never rely on inferred DDL) with server-side `LOAD DATA INFILE` in strict mode (the project-wide default, so any coercion aborts the load); `mysqlsh util.importTable` is used only for tables of 5 M rows or more (Chicago full, Stack Exchange dba `posthistory`, Wikipedia `pagelinks`), where its nonrestrictive LOCAL semantics are accepted because the S4 digests cover every column — `importTable`'s own summary line must report zero warnings (verify at P-04 that the utility prints one).
