...
* Every database gets a `_meta` table? **No** — instead one shared database `megasamples` holds `datasets` (name, tier, upstream version, license, sha256 of source artifact, row-count manifest) so provenance is queryable from SQL.
* Accounts (created by `docker/init/00-users.sql` in the builder stage, re-applied idempotently by the entrypoint wrapper on every start so env-var passwords take effect):
  ...
  * `admin` (default `admin`, override `ADMIN_PASSWORD`): `GRANT ALL PRIVILEGES ON *.* WITH GRANT OPTION` plus dynamic `ROLE_ADMIN, SET_ANY_DEFINER` ([privileges](/sources/mysql-refman-9-7-privileges-provided.md)); `root` stays as the official image defines it (`MYSQL_ROOT_PASSWORD`).
