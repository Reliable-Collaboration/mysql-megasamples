...
* Every database gets a `_meta` table? **No** — instead one shared database `megasamples` holds `datasets` (name, tier, knowledge record path, upstream version, `licenses` JSON array of license ids, `artifacts` JSON array of {id, sha256, size, source}, `row_counts` JSON, build id) so provenance is queryable from SQL; `megasamples/registry.py` is its only writer.
* Accounts (created by `engines/mysql/init/00-users.sql` in the builder stage; at run time `entrypoint-wrapper.sh` applies `DEMO_PASSWORD`/`ADMIN_PASSWORD`/`MYSQL_ROOT_PASSWORD` overrides through a generated `--init-file`, see [bake decision](/decisions/bake-data-vs-initdb.md)):
