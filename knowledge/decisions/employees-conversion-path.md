...
# Options considered
1. Run `mysql < employees.sql` from inside the checked-out directory (works only when the client's CWD is the repo root; the entrypoint's init loop does not `cd`).
2. Generate one file: header of employees.sql (DDL + views) + the eight `load_*.dump` files in the same order + optional objects.sql, with `source` and `flush binary logs` lines removed; compress as `.sql.zst` for the init directory.
3. `employees_partitioned.sql` variant (partitioned salaries/titles/dept_emp) - no advantage for a sample image.

# Evidence
* `source load_departments.dump ;` etc. are client commands with relative paths; `flush /*!50503 binary */ logs;` needs RELOAD and is pointless at init ([employees.sql](/sources/github-datacharmer-test-db-employees-sql.md)).
