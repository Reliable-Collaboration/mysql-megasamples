...
Three consoles ship, not four. `docker compose --profile console up -d` starts nginx serving a
generated landing page on 8080, phpMyAdmin on 8081, Adminer on 8082 and DbGate on 8083, all bound to
`127.0.0.1`, all connected as the read-only `demo` account. Without the profile, `docker compose up`
starts only the database, and nothing is baked into the published image.

**CloudBeaver is dropped**, on the rule this decision already set: it cannot be brought up without
its first-launch wizard, and every documented route was tried
([the question](/questions/console-preconfiguration-limits.md)).
...
and sizes -- 248 tables and 9,056,697 rows -- and cannot drift. `tests/console_test.py` is S10: four
endpoints answering 200, every registry name present on the page, and the credentials on it, because
Adminer's login form is not preset.
...
# Status
accepted. Three of the four consoles ship; CloudBeaver is dropped because it cannot start unattended, which is the condition this decision set for it. The [preconfiguration question](/questions/console-preconfiguration-limits.md) is answered.
