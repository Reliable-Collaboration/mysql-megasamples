...
The landing page is generated by `megasamples/console_page.py` from `megasamples.datasets` inside the
running image, so it lists the 21 databases actually present with their real table counts, row counts
and sizes -- 248 tables and 9,056,697 rows -- and cannot drift. `megasamples/console_test.py` is S10: five
