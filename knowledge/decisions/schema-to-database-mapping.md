...
description: MySQL has no schema level below the database, so AdventureWorks (5 schemas) and WideWorldImporters (4 data schemas + procedure schemas) need a naming rule; options are schema-prefixed table names in one database, one MySQL database per schema, or dropping the prefix. Outcome pending coordinator; recommendation is schema-prefixed names in one database.
resource: /decisions/schema-to-database-mapping.md
tags: [decision, naming, adventureworks, wideworldimporters, pending]
status: stable
trust: inferred
