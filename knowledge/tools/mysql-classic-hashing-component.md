...
# Limits
* `MD5()`/`SHA1()` are unavailable on 9.6+ unless the component is installed; the image does not install it, so every test uses `SHA2()` ([checksum method](/decisions/test-checksum-method.md)).
