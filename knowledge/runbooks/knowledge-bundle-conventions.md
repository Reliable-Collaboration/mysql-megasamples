...
description: How every record in this OKF v0.2 bundle is written, typed, trusted, sectioned, indexed and logged; the checker in megasamples/okf_check.py enforces the rules marked [checked].
...
The `# Attribution` section of a License record holds the exact wording the repository ships (or "None required" plus any courtesy line); `megasamples/provenance.py` reads it. The `# Applied to` section lists every dataset or tool record that links to the license.
...
* Every directory has an `index.md` generated by `megasamples/okf_check.py --write-index` (no frontmatter except the root's `okf_version: "0.2"` and `bundle_status`); the root intro paragraph between `<!-- intro -->` markers is preserved across regenerations. A hand-edited index that differs from the generated form fails the check.
